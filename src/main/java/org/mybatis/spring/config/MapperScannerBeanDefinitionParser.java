  ...
  public synchronized BeanDefinition parse(Element element, ParserContext parserContext) {
    ...
    try {
      ...
      if (StringUtils.hasText(nameGeneratorClassName)) {
    	//Spring2.5下的org.springframework.beans.BeanUtils没有
  		//instantiateClass(Class<?> clazz, Class<T> assignableTo)方法
  		//变通实现.先验证是否为子类,在进行实例化
        Class<?> nameGeneratorClass = classLoader.loadClass(nameGeneratorClassName);
        Assert.isAssignable((Class<?>) nameGeneratorClass, BeanNameGenerator.class);
        BeanNameGenerator nameGenerator = (BeanNameGenerator) BeanUtils.instantiateClass((Class<?>) nameGeneratorClass);
